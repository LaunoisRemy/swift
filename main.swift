...
class Partie : TPartie{
    ...
    func affichage() -> [[String]]{
    	var res : [[String]]=[]
    	print("y/x \t0 \t1 \t2 \t3 \t4")
    	for y in 0..<self.grille.count{
    		var ligne : [String] = [String(y)]
    		for x in 0..<self.grille.count{
    			if(self.grille[y][x].estOccupee){

    				if j2.existePion(x:x,y:y){
    				let pion : TPion = j2.getPion(x:x,y:y)
	    				if(pion.type=="maitre"){
	    					ligne.append("\u{1B}[31m\u{25CF}\u{1B}[0m")	
	    				}else{
	    					ligne.append("\u{1B}[31m\u{25CB}\u{1B}[0m")
	    				}
    				}
    				if j1.existePion(x:x,y:y){
    				let pion : TPion = j1.getPion(x:x,y:y)
    					if(pion.type=="maitre"){
    						ligne.append("\u{1B}[34m\u{25A0}\u{1B}[0m")	
    					}else{
    						ligne.append("\u{1B}[34m\u{25A1}\u{1B}[0m")
    					}	
    				}
    			}else{
    				if x==2 && y==0 {
    					ligne.append("\u{1B}[32m\u{25C8}\u{1B}[0m")	
    				}else {
    					if x==2 && y==4 {
    						ligne.append("\u{1B}[33m\u{25C8}\u{1B}[0m")	
    					}else{
    						ligne.append("\u{2B2B}")
    					}
...
while (!p.finPartie()) {
	print("\n\n============================")
	if(p.joueurCourant.couleur=="bleu"){
		print("Joueur \u{1B}[34mbleu\u{1B}[0m  à toi de jouer ! \n ")
	}else{
		print("Joueur \u{1B}[31mrouge\u{1B}[0m  à toi de jouer ! \n ")
	}
	aff()
    ...
    if(p.joueurCourant.existeDeplacement(partie:p)){ //vérifie si un déplacement est possible avec les cartes qu'il possède
        ...
        repeat {
            ...
            repeat {
                ...
                var v : Int? = nil
                while v == nil {
                    print("il faut une valeur entière pour x : ", terminator:"")
                    v = Int( readLine() ?? "" )
                }
                x1 = v! // v != nil
                ...
                var w : Int? = nil
                while w == nil {
                    print("il faut une valeur entière pour y : ", terminator:"")
                    w = Int( readLine() ?? "" )
                }
                y1 = w! // w != nil
            ...
            repeat {
                print("Il faut saisir le nom d'une des cartes parmis celles proposées : ", terminator:"")
        ...
        repeat {
            ...
            var w : String? = nil
            while w == nil || w == "" {
                print("Il faut saisir un String pour le nom de la carte: ", terminator:"")
