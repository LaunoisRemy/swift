...
func Partie(){
	...
	while !estFini {
		...
		while (!aJoue){
			...
			var piece : Piece = stringToPiece(nom : pieceString )
			...
			var peutPlacer : Bool  = jeu.PeutPlacer(j : jeu.jCourant, p : piece, x : x, y : y)
...
func jeuToString(j : Jeu) -> [[String]]{
	...
	for x in 0...3{
		for y in 0...3 {
			var pTmp : Piece? = j.getPiece(x : x ,y : y)
			if let piece = pTmp {
